defmodule Bumblebee.Text.JinaBertV2 do
  ...
  defp encoder(
    ...
    blocks(
      ...
      [
        ...
        block_type: &jina_block_impl(&1, &2, spec, &3),
  ...
  def jinabertmlp(hidden_states, spec, opts) do
    name = opts[:name]

    hidden_states
    |> Axon.dense(spec.intermediate_size, name: join(name, "up_layer"))
    |> Axon.activation(spec.activation)
    |> Axon.dropout(rate: spec.dropout_rate)
    |> Axon.dense(spec.hidden_size, name: join(name, "down_layer"))
  end
  ...
  def glumlp(
    ...
    hidden_states =
      hidden_states
      |> Axon.dense(intermediate_size * 2, use_bias: false, name: join(name, "up_gated_layer"))
    ...
    hidden_states =
      Axon.multiply(up, gated)
      |> Axon.dropout(rate: hidden_dropout_prob)
      |> Axon.dense(hidden_size, name: join(name, "down_layer"))
  end
  ...
  def multi_head_attention(query, key, value, opts) do
    ...
    query =
      ...
      |> Axon.layer_norm(epsilon: opts[:layer_norm_epsilon], name: join(name, "layer_norm_q"))
    ...
    key =
      ...
      |> Axon.layer_norm(epsilon: opts[:layer_norm_epsilon], name: join(name, "layer_norm_k"))
  ...
  defp jina_block_impl(hidden_state, steps, spec, name) do
    ...
    residual =
      ...
      |> Axon.layer_norm(epsilon: spec.layer_norm_epsilon, name: join(name, "layer_norm_1"))

    mlp_output = steps.ffn.(residual)

    hidden_state =
      Axon.add(residual, mlp_output)
      |> Axon.layer_norm(epsilon: spec.layer_norm_epsilon, name: join(name, "layer_norm_2"))
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        ...
        "encoder.blocks.{n}.ffn.down_layer" => "encoder.layer.{n}.mlp.down_layer",
        "encoder.blocks.{n}.ffn.layernorm" => "encoder.layer.{n}.mlp.layernorm",
        "encoder.blocks.{n}.layer_norm_1" => "encoder.layer.{n}.layer_norm_1",
        "encoder.blocks.{n}.layer_norm_2" => "encoder.layer.{n}.layer_norm_2",
        "encoder.blocks.{n}.ffn.gated_layers" => "encoder.layer.{n}.mlp.gated_layers",
        "encoder.blocks.{n}.self_attention.layer_norm_k" =>
          "bert.encoder.layer.{n}.attention.self.layer_norm_k",
        "encoder.blocks.{n}.self_attention.layer_norm_q" =>
          "bert.encoder.layer.{n}.attention.self.layer_norm_q",
        "encoder.blocks.{n}.ffn.up_gated_layer" => "encoder.layer.{n}.mlp.up_gated_layer"
      }
    end
  end
end
